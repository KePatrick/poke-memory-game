...
function infoEvolute(x,stage){
  ...
  if (stage==2){
    ...
    document.getElementById("info1").innerText = "一階型態"
    document.getElementById("info2").innerText = "三階進化"}
  if (stage==1){
    ...
    console.log('退化:'+pokeInfoData[`${x+1}`]['flipOrNot'])
    console.log('進化'+pokeInfoData[`${x+2}`]['flipOrNot'])
    if(pokeInfoData[`${x+1}`]['flipOrNot']==1){
      brightness('befor')
    }
    else{
    ...
    document.getElementById("info1").innerText = "二階進化"
    document.getElementById("info2").innerText = "三階進化"
  }
  if (stage==3){
    document.getElementById('fig1').style.display = 'flex'
...
// function resetPos(){
//   let elementPosition = getPosition(document.getElementsByClassName('memory-game')[0]);
//   let pointPosition = getPosition(document.getElementById("point"));
//   let positionADJ = pointPosition - elementPosition;
  
//   document.getElementById("point").style.marginLeft = (`${elementPosition+250}px`)
  
  
// }
// resetPos()
// let abc = setInterval(resetPos,100)
...
function flipCard() {
  ...
  // console.log(fliped)
  // console.log(pokeInfoData[`${fliped}`]['pokeID'])
  // console.log(pokeInfoData[`${fliped}`]['flipOrNot'])
  // 避免翻同一張牌當做第二張
  if (this === firstCard) return;
  if (this === secondCard) return; //+
  this.classList.add('flip');
  
  // a=document.getElementsByClassName('flip')
  // console.log(a)
  if (hasFlippedCard == 0) {
    hasFlippedCard = 1;
  ...
  if (hasFlippedCard == 1) {
    ...
    if (((firstCard.dataset.framework == 'PIKA' &&  secondCard.dataset.framework == 'PIKA')&&firstCard.dataset.stage == '10'&& secondCard.dataset.stage == '20')||
    ((firstCard.dataset.framework == 'FOX' &&  secondCard.dataset.framework == 'FOX')&&firstCard.dataset.stage == '10'&& secondCard.dataset.stage == '20')
    ){
      specialcase()
    }
    return;
...
function checkForMatch() {
  ...
  if (((firstCard.dataset.framework == 'PIKA' &&  secondCard.dataset.framework == 'PIKA')&&firstCard.dataset.stage == '1'&& secondCard.dataset.stage == '2')||
  ((firstCard.dataset.framework == 'FOX' &&  secondCard.dataset.framework == 'FOX')&&firstCard.dataset.stage == '1'&& secondCard.dataset.stage == '2')){
 ...
 (thirdCard.dataset.framework == 'FOX' &&  secondCard.dataset.framework == 'FOX')
  ...
  &&firstCard.dataset.framework === thirdCard.dataset.framework&&firstCard.dataset.stage == '1'&& secondCard.dataset.stage == '2'&&thirdCard.dataset.stage== '3';
...
$.get("/leaderboardhard", function (e){
...
function wincounter(){
  ...
  if (wincount==9){
    ...
    await $.get("/leaderboardhard", function (e){
    ...
    a()
//     leaderboardData.push({'name':`${leaderboardname}`,'score':`${progress/2}`});
